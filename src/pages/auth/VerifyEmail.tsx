    ...
    const verifyAccount = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          setIsSuccess(true);
          notice.show("success", "Xác thực email thành công!");
          setTimeout(() => navigate("/"), 2000);
        } else {
          setIsSuccess(false);
          notice.show("error", "Xác thực email thất bại. Vui lòng thử lại!");
        }
      } catch (error) {
        setIsSuccess(false);
        notice.show("error", "Xác thực email thất bại. Vui lòng thử lại!");
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-gray-800 rounded-lg shadow-xl p-8 text-center">
        ...
        <p className="text-gray-300 mb-6">
          {isSuccess
            ? "Email của bạn đã được xác thực thành công. Bạn sẽ được chuyển hướng về trang chủ trong giây lát."
            : "Xác thực email thất bại. Vui lòng kiểm tra lại email hoặc yêu cầu gửi lại email xác thực."}
        </p>
        ...
        <div className="space-y-4">
          ...
          {!isSuccess && (
            <button
              ...
              className="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300"
            >
